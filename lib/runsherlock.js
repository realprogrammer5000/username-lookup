...
module.exports = async (username, timeout, real, page, pageSize, maxCacheSeconds) => {
    ...
    const cachePath = path.join(cacheDir, cachePathName);
    ...
    if(stats && (new Date() - stats.birthtime) / 1000 < maxCacheSeconds){
        // nothing to do, the file's already cached
        console.log(`cache hit: ${cachePathName} was ${(new Date - stats.birthtime) / 1000} seconds old out of ${maxCacheSeconds}`);
    }else {
        console.log("cache miss: " + cachePathName + (stats ? ` was ${(new Date - stats.birthtime) / 1000} seconds old out of ${maxCacheSeconds}` : " was not in cache"));
        ...
        if(real) await fs.rename(csvFilePath, cachePath);
    }

    const resData = (await fs.readFile(cachePath)).toString();
    ...
    parsed.forEach(data => {
        ...
        data.unreliable = ["YandexCollection", "Investing.com", "4pda", "Houzz"].includes(data.name);
    });
    return parsed;
};
